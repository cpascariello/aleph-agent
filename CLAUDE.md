...
- `docs/phase2-context.md` — **Phase 2 build context**: SDK API surface, CLI issues found during testing, account setup, verified SDK call patterns, and gotchas. Read this before any Phase 2 implementation work.
- `.claude/plans/elegant-cooking-clock.md` — **Phase 2 implementation plan**: MCP server architecture, tool definitions, safety controls, directory structure, and implementation order.
...
- **Primary interface**: `aleph-client` CLI (`pip install aleph-client`)
...
2. **Phase 2 (NEXT)**: Python MCP Server (FastMCP) wrapping the Aleph Python SDK directly (not CLI). Safety controls: spending limits, TTLs, inventory management. Plan: `.claude/plans/elegant-cooking-clock.md`
...
- `aleph instance create` requires `--payment-type credit`, a CRN target (`--crn-url` or `--crn-hash`), and SSH key
